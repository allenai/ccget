...
*Updated on 2025-04-08 by Luca Soldaini (lucas@allenai.org)*

First, create a manifest file. Make sure to use `--ignore-cache` to re-download the manifest file if you are
not sure how fresh the cache is.

```bash
uv run scripts/replicate_warc_paths.py -b ai2-russella -s CC-NEWS
```

Then, use the manifest file to copy the shard:

```bash
uv run scripts/copy_shard.py \
    -s 'CC-NEWS/<year>/<month>'
...
Make sure to adjust `<year>` and `<month>` to the correct values;
`<destination-bucket>` is the name of the bucket you want to copy the shard to;
`<arn-role>` is the ARN of the role you want to use to copy the shard.
You can use the script `scripts/create_role.py` to create a role with the appropriate permissions.

## About manifest files (generated by ccget)
